...
<html lang="en">
    ...
    <script>
        ...
        if (value <= 50)
                return 'aqi-value-good';
        if (value <= 100)
                return 'aqi-value-average';
        if (value <= 150)
                return 'aqi-value-poor';
        if (value <= 200)
                return 'aqi-value-bad';
        if (value <= 300)
                return 'aqi-value-very-bad';
        ...
        ws.onopen = () => {
        console.log("Connected to IoT Alert WebSocket");
        };
        ws.onmessage = (event) => {
        const data = JSON.parse(event.data);
        if (data.alert > 0) {
        console.log("🚨 ALERT RECEIVED:", data);
        // Ví dụ: Hiển thị popup
        showAlert(data);
        // Nếu có audio
        if (data.audio_url) {
    ...
    <script>
    let ws = new WebSocket("ws://127.0.0.1:9200");
    ...
    ws.onmessage = function (event) {
        console.log("📩 DATA FROM PYTHON:", event.data);

        let data = JSON.parse(event.data);

        // Nếu là alert thì hiện popup
        if (data.alert > 0) {
            alert("⚠ CẢNH BÁO: " + data.message);

            // Auto play audio từ API python
